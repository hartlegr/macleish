...
#  addPolylines(data = pluck(macleish_layers, "streams")) %>%
  addPolylines(data = pluck(macleish_layers, "boundary"), col = "black")%>%
...
```{r}
treeplot <- base_plot %>%
  addPolygons(data = forests1, color = ~pal(VegType_21))
...
```{r}
  cowIcon <- makeIcon(iconUrl = "http://icons.iconarchive.com/icons/iconsmind/outline/128/Cow-icon.png", iconWidth = 20, iconHeight = 30)
```
